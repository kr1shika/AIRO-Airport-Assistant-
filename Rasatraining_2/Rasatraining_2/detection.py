...
class SerialThread(threading.Thread):
    def __init__(self):
        ...
        self.command = None
        try:
            self.ser = serial.Serial('COM5', 115200, timeout=.025, write_timeout=.025)  # Adjust to your Arduino's serial port
...
def detect_lines(image):
    ...
    if contour is not None:
        ...
        if M["m00"] != 0:
            ...
            print(cx)
            if 45 < cx < 80:
                send_wheel_command(-64, -60)  # Move forward
            elif cx <= 45:
                send_wheel_command(-60, -55)  # Turn left
            elif cx >= 80:
                send_wheel_command(-55, -60)  # Turn right
            # elif cx <= 45:
            #     send_wheel_command(-60, -60)  # Turn left
            # elif cx >= 80:
            #     send_wheel_command(-60, -60)  # Turn right
...
def decode_qr_code(frame, last_detected_time, cooldown, allow_detection):
    ...
    for qr_code in qr_codes:
        ...
        if qr_data == "initial location":
            ...
            stop_robot = True
            stop_duration = 20  # Pause for 20 seconds
            last_detected_time = current_time
        elif qr_data == "gate B":
            print("Detected QR code 'gate B'. Pausing robot for 20 seconds.")
            stop_robot = True
            stop_duration = 20  # Pause for 20 secondsq
            last_detected_time = current_time
        ...
        while self.running:
            ...
            if self.stop_robot:
                ...
                if elapsed_time < self.stop_duration or self.stop_duration == float('inf'):
                    ...
                    if stop_duration == 20 and elapsed_time == 0:  # To ensure the text is spoken only once when it stops
                    ...
                    send_wheel_command(-70, 50)  # Turn with the specified values
                    time.sleep(2.7)  # Stop for 2 seconds after turning
